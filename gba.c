...
void waitForVBlank(void) {
  ...
  // Finally, increment the vBlank counter:
    ...
    while (SCANLINECOUNTER > 160) { // in vBlank
    
    }
    while (SCANLINECOUNTER < 160) { // in vDraw
...
void setPixel(int row, int col, u16 color) {
  // TODO: IMPLEMENT
  //uint16_t videobuffer[];                                       // array of 16 bit integer (color is 16 bits)
  //videoBuffer(row, col) = videobuffer + (row * WIDTH + col);    // WIDTH = 240
...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  // TODO: IMPLEMENT
  DMA[3].src = &color;                                         // source is addr of color val
  for (int i = 0; i < height; i++) {
    DMA[3].dst = &videoBuffer[OFFSET(row + i, col, WIDTH)];    // destination is video memory
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;            // width of rect, enable DMA (fixed source so copies the color)
  }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  // TODO: IMPLEMENT
  DMA[3].src = image;                           // src = address of img data
  DMA[3].dst = videoBuffer;                     // dest = vid memory
  DMA[3].cnt = (WIDTH * HEIGHT) | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;   // count = number of pixels
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  // TODO: IMPLEMENT
  for (int i = 0; i < height; i++) {
    DMA[3].src = &image[OFFSET(i, 0, width)];                                                // source is start of curr img line
    DMA[3].dst = &videoBuffer[OFFSET(row + i, col, WIDTH)];                                  // destination is video memory
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;          // width of img line, enable dma (increment source so copies each element)
  }
}
...
void fillScreenDMA(volatile u16 color) {
  // TODO: IMPLEMENT
  DMA[3].src = &color;                                                                          // src is color val
  DMA[3].dst = videoBuffer;                                                                     // dest is video buffer
  DMA[3].cnt = WIDTH * HEIGHT | DMA_ON | DMA_16 | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT; // (control) enable DMA, 16 bit transfers, fixed source, increment dest
}
